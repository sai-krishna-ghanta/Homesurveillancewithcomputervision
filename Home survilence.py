# opencv is used for face_detection
# datetime is used for storing the exact time at which the face is detected
# argparse is used to combine all the frames where the image is detected
# os is used for file handling
import cv2, time
from datetime  import datetime
import argparse
import os
#casacde consists elements in an object
face_casacde=cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
# capturing video of webcam
video = cv2.VideoCapture(0)
#For image detection,When the image is detected the file is stored in the same folder
while True:
